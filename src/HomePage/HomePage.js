...
function HomePage() {
  ...
  const fetchTableData = () => {
    ...
    axios
      .get(
        `https://167.172.24.111/api/get-table-data/${selectedYearTable}/${storedUserId}`
      )
      .then((response) => {
  ...
  const fetchLineData = (userId, formattedStartMonth, formattedEndMonth) => {
    axios
      .get(
        `https://167.172.24.111/api/get-line-graph-data-range/${usedselectedCategory}/${startYear}/${formattedStartMonth}/${endYear}/${formattedEndMonth}/${userId}`
      )
      .then((response) => {
    ...
    <div className="pagecontent" style={{ display: "flex" }}>
      <div className="homepage-container" style={{ paddingLeft: "25%" }}>
        <div className="tablecontent" style={{ width: "100%", paddingBottom: "4cm" }}>
          ...
          >
            <label style={{ display: "flex", padding: "10px", width: "100%" }}>
              Select Table Year:
              <input
                type="number"
